...
haiku(執筆)
   ↓
node affiliate/lint.mjs   ← 機械的違反を確定検出。トークン0・見落としなし
   ↓
指示役のレビュー           ← lintのWARN(数値・表現)と、渡した事実との矛盾を見る
   ↓
commit / push
```

**lintを通さずにcommitしない。** ERRORが1件でもあれば `exit 1` になる。
lintが弾くのは「判断が要らない違反」だけなので、**残りは必ず人/上位モデルが見る**必要がある。
特に**数値と固有名詞**は機械では正誤を判定できない。ここが最後の砦。
