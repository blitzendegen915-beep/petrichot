...
読むだけでは身につかないので、この場で作れるようにしました。4つの欄を埋めると、そのまま貼り付けられるプロンプトができます。

**まず「例を入れる」を押して、どう変わるかを見てください。** そのあと、自分の用途に書き換えるのが早いです。

```widget prompt-builder
```

全部埋める必要はありません。埋めた項目だけが反映されます。慣れてきたら、自分がよく使う条件をメモしておいて使い回すと、毎回考えずに済みます。
