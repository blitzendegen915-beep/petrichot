...
それぞれ得意なことが違うので、「今やりたいことは、どの系統のAIに頼めばいいのか」を意識できると、ツール選びで迷いにくくなります。やりたいことから逆引きすると、次のように整理できます。

| こんなことをしたい | 頼るべき系統 |
|---|---|
| 文章の意味を調べたい、下書きを直したい | 文章を生成するAI |
| 頭の中のイメージを絵にしたい | 画像を生成するAI |
| 話した内容を記録・整理したい | 音声を認識・生成するAI |
| 録音を文字にして、さらに要点だけ知りたい | 音声認識と文章生成を組み合わせる |

一番下の例のように、複数の系統をまたいで使うケースもあります。この点は記事の後半でもう一度取り上げます。ここから一つずつ見ていきましょう。
...
たとえば読書感想文の書き方に迷ったときは、「答えをそのまま書いてもらう」のではなく、次のように聞くと考えを整理する手助けになります。

```
「銀河鉄道の夜」を読んで感想文を書きたいのですが、何から考えればいいか分かりません。
物語のどんな場面に注目すると、感想が書きやすくなりますか。3つほど提案してください。
```

こう聞けば、AIが感想文そのものを書くのではなく、自分で考えるためのきっかけを渡してくれます。実際にどんなことができるか気になった人は、下のリンクから公式サイトを覗いてみてください。
...
## 系統をまたいで使うときに気をつけたいこと

### 組み合わせて使うことも増えている

ここまで「文章」「画像」「音声」の三系統を分けて説明しましたが、実際のツールはこれらを組み合わせて使えるようになっていることも増えています。たとえば、会議の音声を文字にして（音声認識）、その文字起こしを要約する（文章生成）といった流れは、複数の系統のAIを連続して使っている例です。「このAIは何ができて、何ができないのか」を系統から考えるクセをつけておくと、新しいツールが出てきたときにも、どんな場面で役立ちそうかを判断しやすくなります。
...
もし生成された内容に違和感を覚えたら、「そのまま直す」より先に、元になった情報（自分が入力した文章や、参照させた資料）に間違いがなかったかを見直してください。入力側の情報が不正確だと、AIの出力もそれに引きずられて不正確になりやすいためです。これらは、どの系統のAIを使う場合でも共通して意識しておきたいポイントです。
